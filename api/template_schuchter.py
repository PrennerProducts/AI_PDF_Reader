...
POS_LINE_RE = re.compile(r"^(?P<position>\d+)\s+(?P<qty>\d+)\s+Stck\s+(?P<label>.+)$")
DIMENSION_RE = re.compile(r"B/H:\s*([0-9]+)\s*x\s*([0-9]+)", flags=re.IGNORECASE)
PRICE_PAIR_RE = re.compile(
    r"([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})\s+([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})$"
)
...
def _extract_description_short(block_lines: list[str], fallback: str) -> str:
    for line in block_lines[1:]:
        ...
        if any(keyword in candidate.lower() for keyword in ("fenster", "tür", "tür", "fixelement", "portaltür", "schiebetür")):
            return candidate
    return fallback
    ...
    while idx < len(lines):
        ...
        while end < len(lines):
            if _is_position_start(lines, end):
                break
            if lines[end].startswith(("Summe Netto", "MwSt", "Summe Brutto")):
                break
            end += 1
        ...
        lv_pos = normalize_line(header_match.group("label"))
