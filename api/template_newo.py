...
def detect(normalized_lower: str) -> bool:
    return "newo-sachbearbeiter" in normalized_lower or ("angebotsnummer:" in normalized_lower and "newo" in normalized_lower)
...
def _extract_page_items(lines: list[str], page_ref: int | None = None) -> list[dict[str, Any]]:
    ...
    for idx, line in enumerate(lines):
        if re.match(r"^\d{3}\s+[0-9]+,\d{2}\s+[A-Za-z]+", normalize_line(line)):
    ...
    for offset, start in enumerate(start_indices):
        ...
        unit_price_raw = None
        line_total_raw = None
        price_pair = PRICE_PAIR_RE.search(header)
        if price_pair:
            unit_price_raw = price_pair.group(1)
            line_total_raw = price_pair.group(2)
            header_tail = PRICE_PAIR_RE.sub("", header_tail).strip(" :")
        else:
            for line in block_lines[1:16]:
                price_match = PRICE_PAIR_RE.search(line)
                if price_match:
